...
library Proposals {
  ...
  function adjustedSupport(Proposal storage proposal, int256 criticalBaseline) public view returns (int256) {
    ...
    int256 totalWeightFixed = FixidityLib.newFixed(int256(proposal.totalWeight));
    ...
    int256 participation = FixidityLib.newFixed(int256(proposal.votes.yes.add(proposal.votes.no).add(proposal.votes.abstain))).divide(totalWeightFixed);
